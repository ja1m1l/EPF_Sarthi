...
interface CloudscapeProps extends HTMLAttributes<HTMLDivElement> {
  colorBottom?: string;
  colorMid?: string;
  colorTop?: string;
  speed?: number;
...
const Cloudscape = ({
  ...
  useEffect(() => {
    ...
    const vertexShaderObject = compileGLSLShader(gl.VERTEX_SHADER, vertexShaderGLSL);
    const fragmentShaderObject = compileGLSLShader(gl.FRAGMENT_SHADER, fragmentShaderGLSL);
    ...
    const resize = () => {
      ...
      canvas.height = Math.max(1, Math.floor(hostHeight * dpr));
    ...
    const render = (now: number) => {
      ...
      const colorBottomRgb = hexToRgbNormalized(settings.colorBottom);
      const colorMidRgb = hexToRgbNormalized(settings.colorMid);
      const colorTopRgb = hexToRgbNormalized(settings.colorTop);
      ...
      gl.uniform3f(colorMidUniformLocation, colorMidRgb[0], colorMidRgb[1], colorMidRgb[2]);
      gl.uniform3f(colorTopUniformLocation, colorTopRgb[0], colorTopRgb[1], colorTopRgb[2]);
