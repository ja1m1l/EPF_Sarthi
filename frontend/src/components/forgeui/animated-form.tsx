...
const AnimatedFormCard = ({ name }: { name: string }) => {
  ...
  return (
    <div className="relative w-full max-w-85">
      <div className="w-full rounded-xl border border-neutral-200/60 bg-white/70 p-1.5">
        <div
          className={cn(
            'relative',
            'flex flex-col gap-1 divide-y divide-neutral-200 rounded-lg',
            'bg-linear-to-b from-neutral-50 to-white',
            'shadow-[inset_0_0_0_1px_#e5e5e5,inset_0_2px_0_0_#ffffff80]',
          )}
        >
          <div
            className={cn(
              'px-3 pt-3 pb-2 text-sm leading-4 tracking-wide text-transparent',
              'bg-linear-to-r from-neutral-700 to-neutral-300 bg-clip-text',
            )}
          >
            Create Account
          </div>
          <div className="flex flex-col gap-2 p-2">
            ...
            >
              <div className="pt-0.5 font-mono text-xs text-neutral-800">
                {password.split('').map((char, index) => (
                  <motion.span
                    ...
                    transition={{
                      duration: 0.1,
                      delay: nameAnimationDuration + 0.5 + index * passwordStaggerDelay,
                      ease: 'easeOut',
                    }}
                  >
                    {char}
      ...
      <div className="absolute bottom-0 left-0 flex h-12.5 w-full items-center justify-around px-6">
        {icons.map(({ icon, key }) => (
          <div
            key={key}
            className="rounded-full bg-linear-to-b from-neutral-300/70 to-neutral-100 p-2"
          >
            {icon}
          </div>
        ))}
...
const AnimatedCheckmarkCircle = ({
  ...
  return (
    <div className="relative">
      ...
      >
        <IoMdCheckmark className="size-2.5 text-white" />
      </motion.div>
    </div>
  );
};
...
const ContainerMask = () => {
  return (
    <>
      <div className="absolute bottom-0 left-0 h-10 w-full bg-linear-to-t from-[#e8f4fb] from-60% to-transparent" />
      <div className="absolute bottom-0 left-0 h-25 w-3 bg-linear-to-t from-[#e8f4fb] from-60% to-transparent" />
      <div className="absolute right-0 bottom-0 h-25 w-3 bg-linear-to-t from-[#e8f4fb] from-60% to-transparent" />
    </>
  );
};
